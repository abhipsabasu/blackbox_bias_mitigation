...
baseline.eval()
with torch.no_grad():
    baseline.load_state_dict(torch.load('mnist_baseline.pth'))
...
def extract_clusterFeatures(train_loader, model_old, DEVICE):
    ...
    # print(model_name)
    for batch_idx, (img, targets, z1, _, _, _, _) in enumerate(train_loader):
        # print(batch_idx)
        img = img.to(DEVICE)
        ...
        if overall_feats is None:
            ...
            # overall_keep = np.vstack([overall_keep, keep])
...
def get_margins(train_loader, model_old, DEVICE):
    ...
    kmeans = GaussianMixture(n_components=K, random_state=42).fit(overall_feats) #KMeans(n_clusters=k, n_init=10).fit(overall_feats)
    groups = kmeans.predict(overall_feats)
    target_nmi = nmi(overall_targets.squeeze().tolist(), kmeans.predict(overall_feats).tolist())
    bias_nmi = nmi(overall_z1.squeeze().tolist(), kmeans.predict(overall_feats).squeeze().tolist())
    print(target_nmi, bias_nmi)
    margins = np.zeros((K, 10))
    ...
    # print(normalized_margins, margins)
    # print('margins', normalized_margins[0])
    return kmeans, margins, normalized_margins

kmeans, totals, all_margins = get_margins(loaders['train'], baseline, DEVICE)
...
for epoch in range(config.base_epochs):
    ...
    for batch_idx, (img, target, bias, group, _, _, index) in enumerate(loaders['train']):
        ...
        bias = bias.to(DEVICE)
        # print(img.type)
        one_hot = F.one_hot(target, num_classes=10).to(DEVICE)
        with torch.no_grad():
            _, _, feats_baseline = baseline(img)
        ...
        # margins = 1-margins[torch.arange(margins.shape[0]), target]
        img = img.to(torch.float32)
        # model = model.to(torch.half)
        # margins = torch.ones_like(margins) * 0.5
        
        logits, probas, x, cosine, _ = model(img, margins, s=8)
        # logits, probas, x = model(img)
        # with torch.no_grad():
        #     logits_baseline, probas_baseline, feats_baseline = baseline(img)
        #     margins = 1-F.softmax(logits_baseline, 1)[torch.arange(logits_baseline.shape[0]), target]
        # cost = torch.nn.CrossEntropyLoss(reduction='none')(logits, target.long()) * margins
        # cost = cost.mean()
        cost = cross_entropy_loss_arc(logits, one_hot.float()) #+ compute_supcon_loss(x, target.float(), DEVICE)
    ...
    with torch.no_grad():
        ...
        for desc in test_envs:
            ...
            if (desc == 'valid') and (acc1 > max_val_acc):
                
                print(f"New best val {acc1} at {epoch}, old acc {max_val_acc}")
                max_val_acc = acc1
                save_state_dict(model.state_dict(), 'mnist.pth')
                # print('Eval Epoch [{}/{}] [{}] Unbiased: {:.2f}% [{}]'.format(epoch, config.base_epochs, desc, avg_acc, acc_desc))
                print('Total [{}]: Acc@1:{:.2f}% Acc@5:{:.2f}%'.format(desc, acc1, acc5))
                calculate_acc = True
end_time = time.time()
